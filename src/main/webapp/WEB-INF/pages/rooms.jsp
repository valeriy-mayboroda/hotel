            ...
            <table class="tg">
                ...
                <c:forEach items="${roomsList}" var="room">
                    <tr>
                        ...
                        <td><a href="<c:url value='/edit/${room.room_id}'/>">Edit</a></td>
                        <td><a href="<c:url value='/remove/${room.room_id}'/>">Delete</a></td>
                    </tr>
                </c:forEach>
            </table>
        </c:if>
            ...
            <form:form action="${addAction}" commandName="room"><%-- commandName room будет отправлен формой и получен в (@ModelAttribute("room") Room room) --%>
                <table>
                    <c:if test="${!empty room.category}">
                        <tr>
                            <td>
                                <form:label path="room_id">
                                    <spring:message text="ID"/>
                                </form:label>
                            </td>
                            <td>
                                <form:input path="room_id" readonly="true" size="8" disabled="true"/>
